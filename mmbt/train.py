...
def get_args(parser):
    ...
    parser.add_argument("--max_price", type=float, default=500)
...
def mean_absolute_percentage_error(y_true, y_pred, args): 
    ...
    f= open(args.savedir + "/label_prediction.txt", "a")

    for i_epoch in range(len(y_true)):
      f.write(str(y_true[i_epoch])+'   '+str(y_pred[i_epoch])  + '\n')
      # print(str(y_true[i_epoch]  * args.max_price)+'   '+str(y_pred[i_epoch] * args.max_price ))
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    ...
    if args.task_type == "multilabel":
        ...
        # metrics["acc"] = r2_score(tgts, preds)
        metrics["acc"] = mean_absolute_percentage_error(tgts, preds, args)
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    tgt = tgt.cuda()
    loss = criterion(out.float(), tgt.float())
    return loss, out, tgt
...
def train(args):
    ...
    # args.batch_sz = 1
    test_metrics = model_eval(np.inf, test_loaders, model, args, criterion)
