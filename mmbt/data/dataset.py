    ...
    def __getitem__(self, index):
        if self.args.task == "vsnli":
            ...
            sentence = (
                ...
                + self.tokenizer(self.data["description"][index])[
        ...
        if self.args.task_type == "multilabel":
            ...
            label[
                [self.args.labels.index(tgt) for tgt in data["price"][index]]
            ] = 1
        else:
            # label = torch.LongTensor(
            #     [self.args.labels.index(self.data["price"][index])]
            # )
            label = torch.FloatTensor(
                [self.data["price"][index] / self.args.max_price]
            )

            # print("label: ", label)

        image = None
        if self.args.model in ["image", "concatbow", "concatbert", "mmbt"]:
            if self.data["modelname"][index]:
                image = Image.open(
                    os.path.join(self.data_dir, self.data["modelname"][index] +'.jpg') 
                ).convert("RGB")
            else:
