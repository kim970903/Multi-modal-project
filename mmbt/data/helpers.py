...
def get_labels_and_frequencies(path):
    ...
    # data_labels = [csv.loads(line)["price"] for line in open(path)]
    df = pd.read_csv(path)
    data_labels = df['price'].tolist()
...
def collate_fn(batch, args):
    ...
    if args.task_type == "multilabel":
        ...
        # Single Label case
        tgt_tensor = torch.cat([row[3] for row in batch]).float()
...
def get_data_loaders(args):
    ...
    dev = JsonlDataset(
        os.path.join(args.data_path,"", "dev.csv"),
        tokenizer,
        transforms,
        vocab,
        args,
